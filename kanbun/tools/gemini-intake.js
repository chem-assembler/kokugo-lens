...
for (const e of (Array.isArray(list) ? list : [list])){
  ...
  e.reading.forEach(r => {
    ...
    if (!yy || !o) return;
    if (o.length === 1 && JOSHI.indexOf(o) >= 0){
      // 助詞1字は、正しくても重なる（魚「うを」＋「を」）。機械では決められないので
      // 直さずに知らせるだけにする。其「その」＋「の」のように本物の誤りも混じる
      if (yy.endsWith(o)) suspect.push(id + ' ' + r.c + '「' + yy + '」＋助詞「' + o + '」（読みが助詞を巻きこんでいないか人が見る）');
      return;
    }
