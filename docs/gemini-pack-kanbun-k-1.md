# Gemini 依頼パック: 漢文の白文に「標準的な訓読」を当てる（故事成語・1／1）
...
**`scratch/gemini-k-1.json`** に書き出すこと。`scratch/` は追跡外なので置いて構わない。
...
|---|---|---|---|---|
| K-01b | 吾盾之堅物莫能陥也 | 9 | — | `hitei`（莫） |
| K-05c | 吾与汝畢力平険指通予南 | 11 | — | — |
| K-11a | 見人方引嬰児而欲投之江中 | 12 | — | [置字:而] |
| K-15c | 動作態度無為而不窃鈇也 | 11 | — | `hitei` 二重否定（無〜而不〜） |
| K-16b | 吾不知所之所以反也 | 9 | — | `hitei`／「所」「所以」 |
| K-18e | 子来何苦沾衣如此 | 8 | 説苑 正諫 | `gimon`（何〜如此） |
