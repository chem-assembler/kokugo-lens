...
> **【C1 実装済み・2026-08-15・v53】** §5 の受け口をすべて入れた。以下は当初の調査文書で、
> **実装で分かったことは §11 に追記してある**（見積もりと食い違った点・§10 の未確認事項の答え）。
> **残りは C2（`.rec-short` の詰め）と C3（台本2〜3本目）。**
...
## 11. C1 の実装記録（2026-08-15・v53）【事実】

**入れたもの**（§5 の提案どおり。新規3・追記3）

| ファイル | 中身 | 規模 |
|---|---|---|
| `kanbun/rec.js` 新規 | 自動再生層。アクション8種 | 約270行 |
| `kanbun/demos.json` 新規 | 台本1本（V-K1 百聞不如一見×L1） | 38行 |
| `kanbun/verify_demos.js` 新規 | 台本↔問題データの機械照合（node） | 約190行 |
| `kanbun/index.html` 追記 | 録画用 CSS（約45行）＋ `<script src="rec.js?v=53">` | 1か所ずつ |
| `kanbun/game.js` 追記 | `window.kanbunApp`（6メンバー） | 約25行 |
| `kanbun/tests.js` 追記 | 版ペアに `['rec.js','index.html']` | 1行 |

### 11-1. §10 の未確認事項に付いた答え

1. **810×1440 での見え方** … `.rec-short` の1列化は効いた（`main` が縦積み・**横スクロールなし**）。
   ただし **§4-1 の予測どおり紙が余る: 701px**（1440 のうち約半分）。**C2 で詰める。**
   実測値: `main` 47〜738 / `#stage` 63〜447（778×384） / `#side` 463〜722。
2. **`isMobile:true` の影響** … 収録の完走・タップとも問題なし。
3. **`el.click()` と実 PointerEvent の差** … `el.click()` で足りた（縦書きでも同じ）。
4. **長い問題での横溢れ** … 6字では起きない。長い問題は未検証のまま。
5. **ゴーストカーソルの色** … 朱（`--accent: #b3410e`）にした。和紙色の上での見えは
   **フレームを目視できていないので未確認**（C2 で判断する）。
6. **`recinfo.json` の版が chem のものになる件** … 未着手（実収録の運用で判断）。

### 11-2. 見積もりと違った点

- **`state.marks`（頭出しの訓点）は `window.kanbunApp` を広げずに書けた。**
  セル→パレットの `click()` を続けて呼ぶだけでよく、受け口は §5-3 の6メンバーのまま。
- **`verify_demos.js` は §5-5 の案より強くできた。** L1 は `tapPos` の進み方まで真似るので
  「読み切れていない台本」を落とせる。判定は **`#btn-grade` のアクションごとに
  `expect: 'ok' | 'wrong'` を宣言させる**形にした（台本全体の最終形だけ見ると、
  **わざと間違えてから直す誤答デモ**の途中の wrong を見落とすため）。
- **`rec.js` は InfoLens より約100行短い**（374 → 約270行）。実行シートが無いので
  `syncSheetOffset` / `visibleBottom` / `waitForPlaybackEnd` を落とせた。

### 11-3. 検証（すべて実測）

- `node kanbun/tests.js` 728/728 ／ `node kanbun/verify_demos.js` 不備なし
- **検査が壊れたときに落ちることを確かめた**（通るだけでは検査にならない）:
  存在しない問題ID・読み切らない台本・字番号の範囲外・L1 に `marks` を書く・
  **判定の宣言と実際の食い違い**（片方だけレ点を打って `expect:'ok'` と書く）の5つで exit 1。
  版をずらすと `tests.js` も落ちる（`rec.js と index.html の版が食い違う: 99,53`）
- **通常利用は無傷**: `?rec=` なしでは `__recState` が付かず `#rec-overlay` も生えない。
  `.recording` なし・header/progress-bar は表示・681問ロード・コンソールエラー0
- **収録の完走**: 810×1440・`format=short` で **`__recState='done'`・10.1秒**。
  読み順バッジ `1,2,6,5,3,4` は `order=[0,1,4,5,3,2]` と一致し、
  書き下し「百聞は一見に如かず」・「○ 全部正しい順に読めました！」まで出た。
  コンソールは `[rec] playing` / `[rec] done` の2行のみ
- **`#btn-grade` の矩形は L3 で 80×31**（§5-4 の地雷＝矩形ゼロでカーソルが左上へ飛ぶ、は回避）。
  台本と同じ経路（セル→パレット→判定）を DOM で踏んで「○ クリア！」を確認
- **`window.game` は未定義のまま**（§1-2 の地雷を踏んでいない）

> **Browser ペインの制約**: ペインが非表示だとフレームを合成しないので
> **スクリーンショットが撮れず、CSS の transition も進まない**。
...
## 13. 1本目を通しで見て直した2つ（2026-08-15・v55）【事実】

**コマを1枚ずつ見るのでは足りない。通しの一覧を作ると別のものが見える。**

```bash
# 2秒ごとに10コマ並べた一覧（尺と間を一目で見る）
"$FF" -i video-scripts/out/<デモ>-short.webm -vf "fps=1/2,scale=240:-1,tile=5x2" -frames:v 1 sheet.png -y
```

1. **冒頭2.2秒に、別の問題が映っていた。**
   0秒のコマが「衆狙皆怒」（荘子）＝ **既定で開く問題**（難度がいちばん低いもの）だった。
   `applyState`（頭出し）を `play()` の中に置いていたため、**`delay=1200` を待つあいだ
   ずっと既定の問題が映る**。短尺は最初の1秒が勝負なので致命的。
   → **`applyState` を delay の前へ移した**（`start()` の中）。
2. **読み終わりが12秒なのに 19.84秒まで続いていた**＝ 尾に6秒の静止画。
   → `step1` の wait 1600→1100・hold 700→500、`step4` の hold 1400→700、
   `step5` の wait 800→300・hold 2200→1500。**19.84 → 16.52秒**。

### 出来上がり

`node tools/record/mux.mjs --video=…-short.webm --size=1080x1920 --out=…/V-K1.mp4`
→ **1080×1920 / h264 / 30fps / 16.53秒 / 345KB**。

### V-K1 で「完成」していないこと

- **ナレーションは無い**（設計どおり第1段は無音・字幕のみ。§6-2）
- **動画は chem の `video-scripts/out/` にある**（両リポジトリとも追跡外）。
  国語の動画をどの台帳に載せるかは未決（chem の `tools/videos.js` は chem 用）
- **公開していない**。投稿は人が決めること
